...
export default function ClassificacaoListaPage() {
  ...
  const load = useCallback(async () => {
    ...
    setItems(data.filter((r) => r.status === "AGUARDANDO_CLASSIFICACAO"));
    setLoading(false);
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Classificação de Romaneios" description="Romaneios aguardando classificação de qualidade" />
      ...
      <p className="text-sm text-muted-foreground">
        {filtered.length} romaneio{filtered.length !== 1 && "s"} aguardando classificação
      </p>
      ...
      <div className="rounded-lg border bg-card overflow-x-auto">
        {loading ? (
          ...
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">Nenhum romaneio aguardando classificação.</TableCell></TableRow>
              ) : filtered.map((r) => (
                <TableRow key={r.id} className="cursor-pointer hover:bg-accent/30" onClick={() => navigate(`/balanca/classificacao/${r.id}`)}>
                  ...
                  <TableCell className="text-right font-mono">{r.pesoLiquidoFisico.toFixed(0)} kg</TableCell>
                  <TableCell className="text-xs">{format(new Date(r.criadoEm), "dd/MM/yyyy HH:mm")}</TableCell>
                  <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                    <Button size="sm" className="gap-1" onClick={() => navigate(`/balanca/classificacao/${r.id}`)}>
                      <ClipboardCheck className="h-3 w-3" /> Classificar
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
